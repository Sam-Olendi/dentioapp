...
function showTooltip (event, toothNumber) {
    var tooltip = $('.svg-tooltip-' + toothNumber),
        shape = $('.' + toothNumber + ' > .tooth'),
        toothPart = $(event.target).data('title');

    tooltip.text(toothPart);
...
Template.patientContentTreatment.rendered = function () {
    ...
    findings: function () {
        ...
        Tracker.autorun(function () {
            for ( var j = 0; j < pendingTeeth.length; j++ ) {
                ...
                if (Findings.find({patient_id: Session.get('currentPatient'), tooth_number: currentNumber, tooth_part: currentPart}).fetch().length) {
                    $(pendingTeeth[j]).css({'fill': '#EF4848', 'fillOpacity': .8});
                }
...
Template.patientContentTreatment.events({
    ...
    'click .svg-tooth-completed': function () {
        // attach the modal functionality
   ...
   'click .js-patient-findings-show-previous': function () {
    ...
    'click .js-patient-findings-add-service': function () {
        ...
        var selectWrapper = $(event.target).prev('.js-patient-findings-form'),
            randomNumber = Math.floor(Math.random() * 100) + 1,
            findings = ['Preventive', 'Extractions', 'Restorations', 'Dentures', 'Crowns and Bridges', 'Orthodontics', 'Surgical Extractions', 'Others'];
        ...
        var newSelect = $('#patient-findings-form-select-' + randomNumber); // new select box selector
        ...
        for ( var i = 0; i < findings.length; i++) {
            newSelect.append('<option class="patient-findings-form-option" value="'+ findings[i]+ '">' + findings[i] + '</option>');
        }
    },

    'click .patient-findings-form-select-remove': function () {
        // delete the newly-generated select box row
